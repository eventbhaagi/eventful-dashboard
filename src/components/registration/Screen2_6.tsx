    ...
    return (
        <div className="w-full max-w-4xl mx-auto p-6 space-y-8">
            <div className="text-center space-y-2">
                ...
                <p className="text-gray-600">Screen 2.6 - Event Type Pricing</p>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="space-y-4">
                    <div className="flex items-center space-x-2">
                        <h2 className="text-xl font-semibold">Event Type Pricing Adjustments</h2>
                    </div>
                    <p className="text-sm text-gray-600">
                        System will auto-calculate premium on standard pricing, depending on Event Type & Performance Type.
                        Select the event types you perform at and specify pricing adjustments.
                    </p>

                    <div className="grid grid-cols-1 gap-6">
                        {eventTypes.map((eventType) => {
                            const isSelected = watch(eventType.key as keyof Screen2_6FormData);
                            const percentageKey = `${eventType.key}Percentage` as keyof Screen2_6FormData;
                            const typeKey = `${eventType.key}Type` as keyof Screen2_6FormData;

                            return (
                                <div key={eventType.key} className="border rounded-lg p-4 space-y-4">
                                    <div className="flex items-start justify-between">
                                        ...
                                        <div className="flex items-center space-x-2">
                                            <input
                                                ...
                                                onChange={(e) => setValue(eventType.key as keyof Screen2_6FormData, e.target.checked)}
                                                className="h-4 w-4"
                                            />
                                            <Label htmlFor={eventType.key} className="text-sm">Perform at this event type</Label>
                                        </div>
                                    </div>
                                    ...
                                    {isSelected && (
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div>
                                                <Label htmlFor={`${eventType.key}Type`}>Adjustment Type</Label>
                                                <Select
                                                    value={typeof watch(typeKey) === "string" ? (watch(typeKey) as string) : "premium"}
                                                    onValueChange={(value) => setValue(typeKey, value as "premium" | "discount")}
                                                >
                                                    <SelectTrigger>
                                                        <SelectValue />
                                                    </SelectTrigger>
                                            ...
                                            <div>
                                                <Label htmlFor={`${eventType.key}Percentage`}>Percentage</Label>
                                                <Input
                                            ...
                                            <div className="flex items-end">
                                                <p className="text-sm text-gray-600">
                                                    {watch(typeKey) === "premium" ? "+" : "-"}{watch(percentageKey) || "0"}% on standard pricing
                                                </p>
                                            </div>
                                        </div>
                                    )}
