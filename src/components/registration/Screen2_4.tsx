    ...
    return (
        <div className="w-full max-w-4xl mx-auto p-6 space-y-8">
            <div className="text-center space-y-2">
                ...
                <p className="text-gray-600">Screen 2.4 - Performance Types & Pricing</p>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="space-y-4">
                    <div className="flex items-center space-x-2">
                        <h2 className="text-xl font-semibold">What types of performances do you offer?</h2>
                    </div>
                    <p className="text-sm text-gray-600">
                        Please provide your pricing for each performance type. Duration and pricing are required for each selected type.
                    </p>
                        ...
                        {performanceTypes.map((type) => (
                            <div key={type.key} className="border rounded-lg p-4 space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <Label htmlFor={`${type.key}Duration`}>Duration (minutes)</Label>
                                        <Input
                                    ...
                                    <div>
                                        <Label htmlFor={`${type.key}Price`}>Pricing</Label>
                                        <div className="flex space-x-2">
                                            <Select
                                                value={watch(`${type.key}Currency` as keyof Screen2_4FormData) || "INR"}
                                                onValueChange={(value) => setValue(`${type.key}Currency` as keyof Screen2_4FormData, value)}
                                            >
                                            ...
                                            <Input
                                                ...
                                                {...register(`${type.key}Price` as keyof Screen2_4FormData)}
                                            />
                                        </div>
                                    </div>
                                </div>
